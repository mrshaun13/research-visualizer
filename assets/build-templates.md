# Build Templates & File Structure

## Default Tech Stack

- **React 18** — component framework
- **Vite 5** — build tool (fast dev server, instant HMR)
- **Recharts** — charting library (bar, line, area, composed, radar, pie)
- **Tailwind CSS 3** — utility-first styling (dark theme default)
- **Lucide React** — icon library

## Dark Theme — MANDATORY

All project components MUST use the hub's dark color palette. **Never use light theme colors.** The `App.jsx` shell generated by `hub-gen.mjs add-project` uses `bg-gray-950 text-white` — section components must match.

| Element | Use | Never use |
|---------|-----|-----------|
| Page/card background | `bg-gray-800/50`, `bg-gray-800/30` | `bg-white`, `bg-gray-50` |
| Primary text | `text-white`, `text-gray-200` | `text-gray-900`, `text-gray-800` |
| Secondary text | `text-gray-400`, `text-gray-500` | `text-gray-600`, `text-gray-700` |
| Borders | `border-gray-700/50`, `border-gray-800` | `border-gray-100`, `border-gray-200` |
| Hover states | `hover:bg-gray-800/30` | `hover:bg-gray-50`, `hover:bg-gray-100` |
| Accent badges | `bg-{color}-500/20 text-{color}-400` | `bg-{color}-100 text-{color}-700` |
| Dividers | `divide-gray-800` | `divide-gray-50`, `divide-gray-100` |
| Chart grid strokes | `#374151` | `#e5e7eb` |
| Chart tick fills | `#9ca3af` | `#6b7280` |

## Recharts Safety Rules

- **Treemap `content` prop**: Recharts passes `undefined` for `name` on intermediate/padding nodes. Always guard: `if (!name || width < 30) return null;`
- **Tooltip `active`/`payload`**: Always check `if (!active || !payload?.length) return null;` before accessing payload data
- **ResponsiveContainer**: Must have explicit `width` and `height` props — never rely on parent CSS alone

## File Structure (Hub-Based)

All projects live inside the Research Hub. **Never create standalone Vite projects.** Structural files (`App.jsx`, directories, registry, config) are generated by `hub-gen.mjs add-project`. The AI writes only creative content (`components/*.jsx`, `data/*.js`).

```
<hubPath>/src/projects/<slug>/
├── App.jsx                    # Generated by hub-gen.mjs add-project (do not write manually)
├── meta.json                  # Written by hub-gen.mjs write-meta in Phase 7
├── components/
│   ├── Overview.jsx           # AI-written section component
│   ├── [SectionName].jsx      # AI-written additional sections
│   └── Sources.jsx            # AI-written methodology, disclaimers
└── data/
    ├── researchData.js        # AI-written research data as ES module exports
    └── glossaryTerms.js       # AI-written glossary terms (Phase 6B)

<hubPath>/src/components/       # Shared components (installed by hub-gen.mjs install-components)
├── GlossaryTerm.jsx           # import { GlossaryTerm } from '../../components/GlossaryTerm'
├── CustomTooltip.jsx          # import CustomTooltip from '../../components/CustomTooltip'
└── InsightCallout.jsx         # import InsightCallout from '../../components/InsightCallout'
```

## Data Schema Templates

### Bar chart data
```javascript
[{ label: 'Group A', metric1: number, metric2: number }, ...]
```

### Heatmap data
```javascript
{ rowLabels: ['Period 1', ...], colLabels: ['Item A', ...], data: [[val, ...], ...] }
```

### Time series data
```javascript
[{ year: number, value: number }, ...]
```

Use `null` for missing data (never 0 for missing).

## Reusable Components

1. **CustomTooltip** — dark themed, shows all payload values
2. **Heatmap** — interactive, color-scaled, hover detail panel
3. **InsightCallout** — colored box with key finding text
4. **GlossaryTerm** — inline term with dotted underline, click-to-open flyout with definition + visible skill prompt

### GlossaryTerm Component

Renders an inline term with a subtle dotted underline. On click, opens a flyout card with a plain-language definition and a **visible, copy-ready research prompt** styled like a terminal/clone box (dark rounded input with monospace text and a clipboard copy button).

**Props:**
- `term` (string) — the glossary key to look up in the `glossary` map
- `glossary` (object) — map of term → `{ definition, researchPrompt }` (passed from project's `glossaryTerms` data)
- `children` (ReactNode) — the inline display text (usually the term itself)
- `accentColor` (string, optional) — Tailwind color name, defaults to project accent

**Flyout card contents (top to bottom):**
1. **Term** in bold at top
2. **Definition** (1-2 sentences, plain language)
3. **Label:** `"Deep dive with Research Visualizer:"` in `text-[10px] text-gray-500 uppercase tracking-wider mb-1`
4. **Prompt box** — the research prompt displayed in a dark rounded container:
   - Container: `bg-gray-950 border border-gray-700 rounded-lg px-3 py-2 flex items-start gap-2`
   - Prompt text: `font-mono text-[11px] text-gray-300 leading-relaxed flex-1` — the full prompt is visible (not hidden)
   - Copy button: clipboard icon (`w-4 h-4`) anchored to the right side of the box, `text-gray-500 hover:text-white cursor-pointer flex-shrink-0 mt-0.5`
   - On click: copies prompt to clipboard, icon swaps to checkmark for 2s, then reverts

**Styling:**
- Idle: `border-b border-dotted` at 40% accent opacity, `cursor-pointer`
- Hover: solid underline, subtle background highlight (`bg-{accent}-500/10`)
- Flyout: `absolute z-50`, dark card (`bg-gray-800 border border-gray-700 rounded-lg shadow-xl`), max-width 380px, `p-4`
- Auto-position: flip above term if flyout would overflow viewport bottom
- Dismiss: click outside (use `useEffect` with document click listener), Escape key, or click another term

**Data file (`data/glossaryTerms.js`):**
```javascript
export const glossaryTerms = {
  "TERM": {
    definition: "Plain-language explanation in 1-2 sentences.",
    researchPrompt: "Research [topic]: [expanded framing that would produce a rich dashboard]"
  }
};
```

**Usage in section components:**
```jsx
import { GlossaryTerm } from '../../components/GlossaryTerm';
import { glossaryTerms } from '../data/glossaryTerms';

// In JSX:
<p>The platform provides an <GlossaryTerm term="SDK" glossary={glossaryTerms}>SDK</GlossaryTerm> for developers.</p>
```

## App Layout Pattern

The `App.jsx` shell is **generated by `hub-gen.mjs add-project`** — do not write it manually. It includes:
- Project title and subtitle in the sidebar header
- Sidebar navigation with icons per section
- Active section state management via `useState`
- Version marker comment

The AI writes only the section components, which should include:
- Filter controls where needed (group, gender, time toggles)
- Each section: title + subtitle + 1-3 chart cards + insight callout

## File Writing Fallback

If `write_to_file` tool doesn't persist reliably, use shell heredoc:
```bash
cat > /path/to/file.js << 'ENDOFFILE'
contents
ENDOFFILE
```
Always verify with `ls -la` after writing.
